...
function changeBackgroundEvent(event) {
  sayHelloButton.addEventListener("click", changeCSS);
}
...
function mouseOverEvent(event) {
  sayHelloButton.addEventListener("mouseout", mouseOver);
}
...
function anotherEventClick(event) {
...
const repeatMyParam = () => {
  //YOUR CODE HERE
  axios.get(`http://localhost:3000/repeat/someString`).then((res) => {
...
const queryTest = () => {
  axios
    .get(`http://localhost:3000/query-test/?school=sherena`)
...
/*
    ...
    console will say i have an empty query

    2: Send more than 1 query on the URL -- what happened? 
    console will change the "name" and "subject"
*/
...
const createFood = (event) => {
  ...
  axios.post(`http://localhost:3000/food`, body).then((res) => {
    for (let i = 0; i < res.data.length; i++) {
      ...
      newFood.textContent = res.data[i];
